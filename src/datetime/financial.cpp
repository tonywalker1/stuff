...
namespace stuff::datetime {
    ...
    {
        // find sunday in nyc
        zoned_time sunday = get_week_start(date::make_zoned(get_nyc_zone(), t));
        ...
        return forex_bells {
            date::make_zoned(get_nyc_zone(), open).get_sys_time(),
            date::make_zoned(get_nyc_zone(), close).get_sys_time()};
    }
